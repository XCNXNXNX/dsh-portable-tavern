...
async function generateCard(ctx, spec, version) {
...
async function generateWorldbook(ctx, spec, card) {
  const route = await resolveRoute(ctx);
  ...
  const parsed = extractJson(result.text) ?? repairJson(result.text);
...
async function chatReply(ctx, card, messages, provider, model) {
  const route = await resolveRoute(ctx);
